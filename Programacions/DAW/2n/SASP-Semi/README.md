# Programació didàctica SASP

## Fitxers d'edició

- `docs/`: contingut principal de la programació.
- `SASP_DAW.ods`: font de les taules de `04`, `05` i `10`.
- `zensical.toml`: configuració del site i ordre de navegació.

## Convenció de noms

- Este projecte reutilitza la nomenclatura canònica de `DAW-Semi` per als noms de `docs/`.
- Si es renombra algun fitxer, cal actualitzar de manera coordinada `docs/`, `zensical.toml`, `tools/sync_ods_tables_site.py`, `rebuild.sh` i `README.md`.
...
- En l'estat actual del projecte, els fitxers afectats són `2-relacio-uc.md`, `3.contribucio_ra.md`, `4.RAs_CAs_Continguts.md`, `5.esquema_general_up.md` i `10.Avaluacio.md`.
- Si s'afigen de nou blocs ODS a altres apartats, cal ampliar `SYNC_TARGETS` de manera coherent.
- Si falta algun marcador en un fitxer referenciat, `./rebuild.sh` fallarà abans de generar el site.
...
- `programacio-didactica_SASP.pdf`: última exportació a PDF.

## Flux recomanat

1. Editar `docs/` i, si cal, `SASP_DAW.ods`.
