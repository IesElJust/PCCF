# Programació didàctica NC

## Fitxers d'edició

- `docs/`: contingut principal de la programació.
- `PD_Normativa.ods`: font de les taules de `04`, `05` i `10`.
- `zensical.toml`: configuració del site i ordre de navegació.

## Convenció de noms

- Este projecte seguix quasi tota la nomenclatura canònica de `DAW-Semi`, amb l'excepció de `2.Marc_normatiu.md`.
- Esta excepció respon al fet que en els cursos d'especialització no hi ha apartat regulat de relació d'unitats de competència.
- Si es duplica a un altre mòdul, convé revisar de manera coordinada `docs/`, `zensical.toml`, `tools/sync_ods_tables_site.py`, `rebuild.sh` i `README.md`.
...
- En este projecte, els fitxers afectats actualment són `4.RAs_CAs_Continguts.md`, `5.esquema_general_up.md` i `10.Avaluacio.md`.
- Si s'afigen nous blocs ODS a altres apartats, cal ampliar `SYNC_TARGETS` de manera coherent.
...
- `programacio-didactica_NC.pdf`: última exportació a PDF.

## Flux recomanat

1. Editar `docs/` i, si cal, `PD_Normativa.ods`.
