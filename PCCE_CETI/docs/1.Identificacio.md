El *Decret 95/2026, de 19 de juny del Consell, pel qual s'establixen els currículums dels cursos d'especialització de Formació Professional per a la Comunitat Valenciana*, establix que cada centre docent desenvoluparà i completarà, en el marc de la seua autonomia, els currículums de cada curs d'especialització que impartisca mitjançant l'elaboració de projectes curriculars (PCCE), els quals suposaran el marc de referència per a la planificació, el desenvolupament i l'avaluació del procés d'ensenyament i aprenentatge del cicle.

# 1. Identificació del cicle formatiu

<!-- 
El  primer  requeriment  d'un  projecte  curricular  és  la  identificació.  Per  a  això  serà necessari especificar la denominació oficial, la família professional i qualsevol dada que identifique el cicle formatiu desenrotllat en el projecte. A més, és necessari concretar el centre educatiu que el desplega amb la seua localització i l'equip educatiu responsable. 
-->

El títol de Màster FP en **Ciberseguretat en Entorns TI** queda identificat pels elements següents:

* **Denominació**: Ciberseguretat en Entorns TI
* **Nivell**: Formació Professional de Grau Superior.
* **Durada**: 720 hores.
* **Equivalència en crèdits ECTS**: 43.
* **Família professional**: Informàtica i Comunicacions.
* **Cicles formatius de referència**: Tècnic Superior en Desenvolupament d'Aplicacions Multiplataforma (DAM), Tècnic Superior en Desenvolupament d'Aplicacions Web (DAW), Tècnic en Administració de Sistemes Informàtics i Xarxes.
* **Branques de coneixement**: Enginyeria i arquitectura.
* **Referent a la Classificació Internacional Normalitzada de l'Educació**: P-5.5.4.
* **Nivell del Marc Espanyol de Qualificacions per a l'Educació Superior**: Nivell 1-C Curs d'Especialització de Grau Superior (Nivell 5C del MECU).

